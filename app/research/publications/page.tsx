...
export default function PublicationsPage() {
  ...
  const [publicationsPage, setPublicationsPage] =
    useState<ResearchPublicationsPage | null>(null);
  ...
  const displayedSections: Section[] = useMemo(() => {
    ...
    if (sort === "alpha") {
      const papers = (publicationsPage.sorted_alphabetically ?? []).map(
        publicationToPaper,
      );
      return papers.length > 0 ? [{ topic: "All Publications", papers }] : [];
    }

    const papers = (publicationsPage.sorted_by_date ?? []).map(
      publicationToPaper,
    );
    ...
    <main className="min-h-screen bg-[#FFFFFF] w-full px-6 py-10 sm:px-8 sm:py-12 md:px-10 md:py-14 lg:px-12 lg:py-16">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="max-w-4xl mx-auto text-center text-[#E79121] font-semibold text-[18px] leading-[1.35] sm:text-[20px] md:text-[22px] mb-10 sm:mb-12 md:mb-14">
          Papers are separated by topic. Representative publications are marked
          with a ☆
        </p>
                ...
                <div className="space-y-2 text-[#459A9F] text-[16px] leading-[1.7] sm:text-[18px]">
                  {section.papers.map((paper, index) => (
                    <p key={`${paper.title}-${index}`}>
                      {paper.featured && (
                        <span className="text-[#E79121]">☆ </span>
                      )}
                      <span className="font-semibold">{paper.prefix}</span>{" "}
                      {paper.link ? (
                        <a
                          href={paper.link}
                          target="_blank"
